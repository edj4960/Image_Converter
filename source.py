...
parser.add_argument('files', metavar='1 source file, 1 target file and 1 output file name', type=str, nargs=3,
	help='takes 1 source file and 1 target file to run the code and one output file name to save the code') 
...
# save models
if denoise == True:
    autoencoder.save(os.path.join(io.DeepLearningRoot(),'savedModels/' + args.files[2] + '_DAE.h5'))                 
calibMMDNet.save_weights(os.path.join(io.DeepLearningRoot(),'savedModels/' + args.files[2] + '_ResNet_weights.h5'))

#np.savetxt('savedModels/' + args.files[2] +'.csv', h5py.File('home/ejones36/Desktop/BatchEffectRemoval-master/savedModels/' + args.files[2] + '_ResNet_weights.h5')['dataname'], '%g', ',')
